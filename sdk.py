...
class UAIP_Enterprise_SDK:
    def __init__(self, agent_name, company_name, secret_code: int, gateway_url="http://localhost:8000"):
    ...
    def call_agent(self, task, amount, intent):
        ...
        # FIX: Ensure amount is sent as a string for the Hardened Gateway
        data = {"task": task, "amount": str(amount), "intent": intent, "nonce": uuid.uuid4().hex, "timestamp": time.time()}
        msg = json.dumps(data, sort_keys=True, separators=(',', ':')).encode('utf-8')
        sig = self.sk.sign(msg).signature.hex()

        packet = {
            "sender_id": self.did, "task": task, "amount": str(amount), 
            "chain": "BASE", "intent": intent, "data": data, "signature": sig, 
            "public_key": self.pk, "nonce": data["nonce"], 
            "timestamp": data["timestamp"], "zk_proof": proof
        }
