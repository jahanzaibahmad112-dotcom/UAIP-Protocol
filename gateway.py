...
app = FastAPI(title="UAIP Fortress Gateway v3.1")
...
async def execute(req: UAIPPacket):
    ...
    with db_session() as conn:
        ...
        # C. COMPLIANCE ACTIVE ENFORCEMENT
        auditor = ComplianceAuditor()
        audit_decision, _ = auditor.run_active_audit({"sender": req.sender_id, "task": req.task, "amount": req.amount})

        if audit_decision == "TERMINATE":
            conn.execute('INSERT OR REPLACE INTO blacklist VALUES (?, ?)', (req.sender_id, now))
            raise HTTPException(status_code=451, detail="TERMINATED_BY_ENFORCER")

        # D. Governance Logic
        decision = "PENDING" if amt_dec >= 1000 or audit_decision == "PENDING_ENFORCED" else "ALLOW"
        req_id = str(uuid.uuid4())[:8]
        
        conn.execute('INSERT INTO action_logs VALUES (?, ?, ?, ?, ?, ?, ?)',
                     (req_id, html.escape(req.sender_id), html.escape(req.task), str(amt_dec), decision, req.chain, now))
        ...
        # E. Settlement
        UAIPFinancialEngine().process_settlement(req.sender_id, amt_dec, "provider_01", req.chain)
...
async def dashboard():
    ...
    rows = "".join([f"<tr><td>{l['sender']}</td><td>{l['task']}</td><td>${l['amount']}</td><td style='color:{'green' if l['decision']=='ALLOW' else 'orange'}'>{l['decision']}</td><td><button onclick=\"auth('{l['id']}')\">Approve</button></td></tr>" for l in logs])
    return f"<html><head><meta http-equiv='refresh' content='5'><style>body{{background:#0d1117;color:white;font-family:sans-serif;padding:40px;}}table{{width:100%;border-collapse:collapse;}}td{{padding:10px;border-bottom:1px solid #333;}}button{{background:#238636;color:white;border:none;padding:8px;cursor:pointer;}}</style></head><body><h1>🛡️ UAIP Master Dashboard</h1><table>{rows}</table><script>async def auth(id) {{ const k = prompt('Admin Key:'); if(!k) return; const res = await fetch('/v1/decision/'+id+'/allow', {{method:'POST', headers:{{'X-Admin-Key':k}}}}); const d = await res.json(); alert(d.status==='SETTLED'?'✅ Success':'❌ Denied'); location.reload(); }}</script></body></html>"
